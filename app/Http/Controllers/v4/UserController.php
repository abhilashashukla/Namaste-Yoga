...
class UserController extends Controller
{
    ...
    public function getUserByCity(Request $request){
      ...
      try{
        ...
        return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), 'message'=>$message, 'data'=>$cityArr ]);
      }catch(Exception $e){
        $message = "Technical error";
        return response()->json(['status'=> Config::get('app.status_codes.NP_INVALID_REQUEST'),'message'=>$message,'data'=>json_decode("{}")]);
      }
  ...
  /**
    ...
    public function otpVerification(Request $request){
      ...
      try{
        ...
        //$validator->errors()
        if($validator->fails()){
          return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=>"Invalid input data.","data"=>json_decode("{}")]);
        }
        ...
        if($user->count()>0){
          if($user[0]->verified_otp != 1){
			...
			*/
            ...
            if($user[0]->role_id == 2) {
				$updateData = ['verified_otp'=>1, 'status'=> '0'];
			} else {
				$updateData = ['verified_otp'=>1, 'status'=> '1'];
			}
            
            User::where('email', $user[0]->email)->update($updateData);
            ...
            if($user[0]->role_id == 2 || $user[0]->role_id == 3){
              $data['email_msg_1'] = 'Your registration details have been sent to the administrator for approval. Once approved, your listing will be visible on the Namaste-Yoga app.';
            }
            ...
            if($this->SendMail($data,'verified_registration')){
              ...
              //notify code ends
              return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), 'message'=>"Successfully verified OTP",'data'=>json_decode("{}")]);
            }else{
              return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_FAIL'), 'message'=>"Mail not sent",'data'=>json_decode("{}")]);
            }
          }else{
            return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_FAIL'), 'message'=>"OTP Already verified",'data'=>json_decode("{}")]);
          }
        }else{
          return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_FAIL'), 'message'=>"Incorrect OTP",'data'=>json_decode("{}")]);
        }
      }catch(Exception $e){
        $message = "Technical error";
        return response()->json(['status'=> Config::get('app.status_codes.NP_INVALID_REQUEST'), 'message'=>$message,'data'=>json_decode("{}")]);
  ...
  /**
      ...
      try{
        ...
        if($user->count()>0){
          if($request->new_password != $request->confirm_password){
            return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_FAIL'), 'message'=>'confirm password is not same','data'=>json_decode("{}")]);
          }
          ...
          if($validator->fails()){
            return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_FAIL'),'message'=>'Password should be min 6 char, alphanumeric,atleast one char capital and one special char','data'=>json_decode("{}")]);
          }

          if($request->old_password == $request->new_password){
            return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_FAIL'),'message'=>'old password and new password should not be same','data'=>json_decode("{}")]);
          }

          //echo $request->old_password."                ".$user->password; die;
          if(!Hash::check($request->old_password, $user->password)){
            return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_FAIL'),'message'=>'old password incorrect','data'=>json_decode("{}")]);
          }

          if(!$this->checkPasswordHistory($request->new_password,$user->id)){
            return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_FAIL'),'message'=>'Password should not same as last 3 password','data'=>json_decode("{}")]);
          }else{
            ...
            if($this->SendMail($data,'password_change')){
              return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'),'message'=>$message, 'data'=>json_decode("{}")]);
            }else{
          ...
          return response()->json(['status'=> Config::get('app.status_codes.NP_NO_RESULT'), 'message'=>'not found','data'=>json_decode("{}")]);
        }
      }catch(Exception $e){
        return response()->json(['status'=> Config::get('app.status_codes.NP_INVALID_REQUEST'), 'message'=>'exception error','data'=>json_decode("{}")]);
      }
  }
    ...
    public function forgotPassword(Request $request){
      ...
      try{
        ...
        //$validator->errors()
        if($validator->fails()){
          return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=>"invalid input.","data"=>json_decode("{}")]);
        }
        $request->email = $this->string_replace("__","/",$request->email);
        $request->merge([
          'email'=>$request->email
        ...
        if($users->count() > 0){
			...
			if($users[0]->suspended == 1) {
				return response()->json(['status'=> Config::get('app.status_codes.NP_ACCOUNT_SUSPEND'), 'message' => 'Account suspended','data' => json_decode("{}")]);
			}
          ...
          $data['name'] = ucwords($users[0]->name);//$this->encdesc($users[0]->name,'decrypt');
          $data['supportEmail'] = config('mail.supportEmail');
          $data['website'] = config('app.site_url');
          $data['site_name'] = config('app.site_name');
          ...
          if($users[0]->save()){
            ...
            return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), 'message'=>'Otp sent','data'=>json_decode("{}")]);
          }else{
            return response()->json(['status'=> Config::get('app.status_codes.NP_DB_ERROR'),'message'=>'DB error or mail not sent','data'=>json_decode("{}")]);
          }
        }else{
          return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_FAIL'), 'message'=>'If the email address is known to us, we will send a OTP in a few minutes.','data'=>json_decode("{}")]);
        }
  ...
  /**
    ...
    public function verifyForgotPassword(Request $request){
      ...
      try{
        ...
        //$validator->errors()
        if($validator->fails()){
          return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'),"message"=>"invalid credential.","data"=>json_decode("{}")]);
        }
        ...
        if($users->count() > 0){
          ...
          if($users[0]->verified_otp==1){
            return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), 'message'=>'Otp already verified','data'=>json_decode("{}")]);
          }
          $userObj = new User();
          $data = array();
          ...
          if($users[0]->save()){
            ...
            $otpHistory->type = 'OV';
            $otpHistory->save();
            return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'),'message'=>'You have successfully reset your password','data'=>json_decode("{}")]);
          }else{
            return response()->json(['status'=> Config::get('app.status_codes.NP_DB_ERROR'), 'message'=>'DB Error or mail not sent','data'=>json_decode("{}")]);
          }
        }else{
  ...
  public function notifyMe(Request $request){
    try{
        ...
        if($validator->fails()){
          return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=>"Invalid data","data"=>json_decode("{}")]);
        }
        ...
        if($data->count() > 0){
          return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=>"You have already requested for this location","data"=>json_decode("{}")]);
        }
        ...
        if($notifyObj->save()){
          ...
          if($this->SendMail($data1,'notify')){
            return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), "message"=>"save successfully","data"=>json_decode("{}")]);
          }
        }else{
          return response()->json(["status"=> Config::get('app.status_codes.NP_DB_ERROR'), "message"=>"error on saving data","data"=>json_decode("{}")]);
        }
        //$validator->errors()
    }catch(Exception $e){
      return response()->json(['status'=> Config::get('app.status_codes.NP_INVALID_REQUEST'), 'message'=>'exception error','data'=>json_decode("{}")]);
  ...
  public function resentRegisterOtp(Request $request){
    try{
        ...
        if($user->count()==0){
          return response()->json(["status"=> Config::get('app.status_codes.NP_NO_RESULT'), "message"=>"User does not exist","data"=>json_decode("{}")]);
          ...
          if($this->SendMail($data,'registration_otp')){
            $user->otp = $otp;
            $user->save();
            return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), "message"=>"Otp sent successfully","data"=>json_decode("{}")]);
          }
        }else{
          return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'),"message"=>"Technical error","data"=>json_decode("{}")]);
        }
        //$validator->errors()
    }catch(Exception $e){
      return response()->json(['status'=> Config::get('app.status_codes.NP_INVALID_REQUEST'), 'message'=>'exception error','data'=>json_decode("{}")]);
