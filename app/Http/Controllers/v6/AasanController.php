...
{   
   public function getSubCategoryList(Request $request)
   {
      ...
      {     
               ...
               else
               {
                  ...
                  {
                     $sub_category_data=[];
                     $sub_category_list=AasanaSubCategory::where('aasana_categories_id',$category->id)->where('status','1')->orderBy('id','desc')->select('id','aasana_categories_id','subcategory_name','subcategory_description','subcategory_image')->get(); 
                     foreach($sub_category_list as $sub_category)
                     {  
						$total_aasana=aasanaModel::where('aasana_sub_categories_id',$sub_category->id)->where('aasana_categories_id',$sub_category->aasana_categories_id)->where('status','1')->count();
                        $sub_category_array=
                        [
						   "total_aasana"=>$total_aasana,
                           "id"=>$sub_category->id,
   ...
   {    
      ...
      {
               $rules = [
			      'aasana_categories_id'    => 'required|numeric',
              ...
              {
                   $aasana_data=aasanaModel::where('aasana_categories_id',$request->aasana_categories_id)->where('aasana_sub_categories_id',$request->aasana_sub_categories_id)->where('status','1')->orderBy('id','desc')->select('id','aasana_categories_id','aasana_sub_categories_id','aasana_name','aasana_description','assana_tag','assana_video_id','assana_video_duration','assana_benifits','assana_instruction')->paginate(Config::get('app.aasana_record_per_page'));
                   $aasana_list=[];
                   ...
                   {  
                       $aasana_list[]= [
                                            "id"=>$aasana->id,
											'aasana_categories_id'=>$aasana->aasana_categories_id,
