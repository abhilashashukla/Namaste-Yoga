...
use App\Models\GeneralDeviceToken;
...
class UserController extends Controller
{
    ...
    public function authenticate(Request $request)
    {
        ...
        if($validationChk->count()==0){
          ...
          return response()->json(["status"=>0,"message"=>"User not verified","data"=>json_decode("{}")]);
        }else if(!Hash::check($request->password, $validationChk[0]->password)){
            return response()->json(["status"=>$status,"message"=>"invalid credentials","data"=>json_decode("{}")]);
        }else if($validationChk[0]->role_id==3 && $validationChk[0]->ycb_approved != '1'){
            return response()->json(["status"=>$status,"message"=>"YCB not approved","data"=>json_decode("{}")]);
        }else if($validationChk[0]->suspended=='1'){
            return response()->json(["status"=>2,"message"=>"Account suspended","data"=>json_decode("{}")]);
        }
    ...
    public function authenticate2(Request $request)
    {
        ...
        $email_decrypted = strtolower($this->encdesc($request->email,'decrypt'));
        $email_encrypted_lc = $this->encdesc($email_decrypted); // encrypt again


        if($validator->fails()){
            // Check for the small case email id - Added by Awnish
            $request->merge([
              'email' => $email_encrypted_lc
            ]);
            print_r($request->email);
            echo '--------140--------\n';
            $validator_lc = Validator::make($request->all(), [
                'email' => 'required|string|max:255',
                'password' => 'required|string',
            ]);
            $request->email = $email_encrypted_lc;
            if($validator_lc->fails()){
              return response()->json(["status"=>$status,"message"=>"invalid input details","data"=>json_decode("{}")]);
            }
        ...
        //echo $pwd = Hash::make($request->password).'      ='.$request->email; die;
      //   $validationChk = User::where('email',$request->email)->get(); // Commented by Awnish
         $validationChk = User::whereIn('email',[$request->email, $email_encrypted_lc])->get();

        if($validationChk->count()==0){
          return response()->json(["status"=>$status,"message"=>"invalid credentials 171","data"=>json_decode("{}")]);
        }else if($validationChk[0]->status != '1'){
          return response()->json(["status"=>0,"message"=>"User not verified","data"=>json_decode("{}")]);
        }else if(!Hash::check($request->password, $validationChk[0]->password)){
            return response()->json(["status"=>$status,"message"=>"invalid credentials 175","data"=>json_decode("{}")]);
        }else if($validationChk[0]->role_id==3 && $validationChk[0]->ycb_approved != '1'){
            return response()->json(["status"=>$status,"message"=>"YCB not approved","data"=>json_decode("{}")]);
        }else if($validationChk[0]->suspended=='1'){
            return response()->json(["status"=>2,"message"=>"Account suspended","data"=>json_decode("{}")]);
    ...
    public function apilogout(Request $request){
      try {
            ...
            //echo'apilogout<pre>';print_r($request->bearerToken());exit;
            //update user device id
            $obj_general_token = new GeneralDeviceToken;
            
            $obj_general_token->device_id = $user->device_id;
            if($user->device_id != '') {
                if($obj_general_token->save()) {
                    DB::commit();
                }

                //update in user table
                User::where('id', $user->id)->update(['device_id' => '']);
                DB::commit();
            }
            ...
            return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), 'message'=>'Logout successfully', 'data'=>json_decode("{}")]);
        }catch(Exception $e){
            return response()->json(['status'=> Config::get('app.status_codes.NP_INVALID_REQUEST'), 'message'=>'Not able to logout', 'data'=>json_decode("{}")]);
        }
    }
    ...
    public function suspendAccount(Request $request){
      try{
        ...
        if($user->save()){
          ...
          return response()->json(['status'=>0,'message'=>'Unable to suspend','data'=>json_decode("{}")]);
        }
        //JWTAuth::setToken($token)->invalidate();
      }catch(Exception $e){
    ...
    public function ResumeOtpAccount(Request $request){
      try{
          $request->email = $this->string_replace("__","/",$request->email);
          ...
          if(isset($user->id)){
             ...
             if($user->suspended=='0'){
              return response()->json(["status"=>0,"message"=>"Account Already Activated","data"=>json_decode("{}")]);
             }
             ...
             $data['subject'] = 'Account Activation OTP from '.config('app.site_name');
             $code = base64_encode($otp."__".$user->email);
             $data['url'] = url('/')."/activate_account?params=".$code;
             $user->suspend_otp = $otp;
             
             if($user->save()){
              $this->SendMail($data,'account_activation_otp');
           ...
           return response()->json(["status"=>$status,"message"=>"invalid Email details","data"=>json_decode("{}")]);
          }
        //JWTAuth::setToken($token)->invalidate();
      }catch(Exception $e){
        return response()->json(['status'=>0,'message'=>'Not able to logout','data'=>json_decode("{}")]);
    ...
    public function ResumeAccount(Request $request){
      try{
        ...
        if(isset($user->id)){
           ...
           return response()->json(['status'=>1,'message'=>'Account Activated','data'=>json_decode("{}")]);
        }else{
          return response()->json(['status'=>0,'message'=>'Invalid Details','data'=>json_decode("{}")]);
        }
        //JWTAuth::setToken($token)->invalidate();
      }catch(Exception $e){
        return response()->json(['status'=>0,'message'=>'Not able to logout','data'=>json_decode("{}")]);
    ...
    /**
     * Adding user "role_id" (role_id 5 for guest user)
      ...
      try {
          ...
          $request->phone = $this->string_replace("__","/",$request->phone);
            ...
            if($request->get('role_id') == 2) {
                $request->sitting_capacity = $request->sitting_capacity;
            } else {
                $request->sitting_capacity = '';
            }
            ...
            $request->merge([
             ...
             $data['name'] = ucwords($request->get('name'));
             $data['supportEmail'] = config('mail.supportEmail');
             $data['website'] = config('app.site_url');
             $data['site_name'] = config('app.site_name');
             ...
             if(isset($userData->email) && $userData->status=='0' && $userData->verified_otp==0){
               ...
               if($this->SendMail($data,'registration_otp')){
                  ...
                  //$userData = $userData;
                  return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_SUCCESS'),"message"=>"OTP sent successfully.","data"=>$userData]);
                  //die;
               }
            }
           ...
           if($validator->fails()){
             ...
             return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'),"message"=>$message,"data"=>json_decode("{}")]);
           }
           ...
           if($request->get('role_id')==3){
              if(isset($request->ycb_number)){
                $ycb_number = $request->get('ycb_number');
              }else{
                return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=>'YCB number is missing', "data"=>json_decode("{}")]);
              }
           }
         ...
         if($this->SendMail($data,'registration_otp')){
           DB::commit();
           return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), "message"=>$message,"data"=>compact('user','token')]);
         }else{
           return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=>'Unable to send email',"data"=>json_decode("{}")]);
         }
       } catch(Exception $e){
         DB::rollBack();
         return response()->json(['status'=> Config::get('app.status_codes.NP_INVALID_REQUEST'), 'message'=>$e, 'data'=>json_decode("{}")]);
       }
    }
    ...
    public function register2(Request $request)
    {
      ...
      try{
          ...
          $request->merge([
           ...
           // echo $request->get('email');
           // echo $this->encdesc($request->get('email'),'decrypt');
           $data = [];
           $data['email'] = $this->encdesc($request->get('email'),'decrypt');
           $data['name'] = ucwords($request->get('name'));
           $data['supportEmail'] = config('mail.supportEmail');
           ...
           // changed by awnish - to validate both uppwer and lower case in email
           // Step 1 - decrpty the email id and convert to lowercase
           // Step 2 - Store only lowerase in the DB
           $email_decrypted = strtolower($this->encdesc($request->email,'decrypt'));
           $email_encrypted_lc = $this->encdesc($email_decrypted);
           $request->email = $email_encrypted_lc;
           $request->merge([
             'email' => $email_encrypted_lc
           ]);

           //$email_encrypted2 = $this->encdesc('test_awn@yopmail.com');
           // echo '-----------'.$email_decrypted.'----------';
           // echo '-----------'.$email_encrypted_lc.'----------';
           // echo '----RE1-------'.$request->email.'----RE---'.$request->get('email').'---';
           $userData  = User::whereIn('email', [$request->email, $email_encrypted_lc])->first();
           // print_r($userData);
           // print_r($data);
           // Change ENDS - AWNISh
           ...
           if($request->get('role_id')==3){
              if(isset($request->ycb_number)){
                ...
                return response()->json(["status"=>$status,"message"=>'YCB number is missing',"data"=>json_decode("{}")]);
              }
           }
        ...
        try{
         ...
         // print_r($cond); die;
          ...
          if($users->count()==0){
            ...
            return response()->json(['status'=> Config::get('app.status_codes.NP_NO_RESULT'), 'message'=>'No record found', 'data'=>[], 'count'=>$users, 'cond'  => $cond ]);
          }else{
          ...
          return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), 'message'=>$message, 'total_record'=>$users->total(), 'last_page'=>$users->lastPage(), 'current_page'=>$users->currentPage(), 'data'=>$users->items(), 'cond'  => $cond ]);
        }catch(Exception $e){
          $message = "Technical error";
          return response()->json(['status'=> Config::get('app.status_codes.NP_INVALID_REQUEST'), 'message'=>$message, 'data'=>json_decode("{}")]);
        }
      ...
      try{
        ...
        if($user->count()>0){
          if($user[0]->verified_otp != 1){
            ...
            if($user[0]->role_id == 3){
              $data['email_msg_1'] = '<br>Your registration details have been sent to the administrator for approval. Once approved, your listing will be visible on the Yoga Locator app<br><br>';
            }
  ...
  public function notifyMe(Request $request){
    try{
        ...
        if($notifyObj->save()){
          ...
          $data1['name'] = ucwords($this->encdesc($request->name,'decrypt'));
  ...
  public function resentRegisterOtp(Request $request){
    try{
        ...
        if($user->count()==0){
          ...
          $otp = rand(111111,999999);
          $data['name'] = ucwords($this->encdesc($user->name,'decrypt'));
          $data['otp'] = $otp;
          $data['email'] = $this->encdesc($user->email,'decrypt');
          $data['supportEmail'] = config('mail.supportEmail');
          $data['website'] = config('app.site_url');
    ...
    public function editMyProfile(Request $request){
        try{
            ...
            $user->sitting_capacity = ($request->sitting_capacity) ? $request->sitting_capacity : $user->sitting_capacity;
            ...
            if(!$user->save()){
                ...
                return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), 'message'=>'Profile updated successfully', 'data'=>$user]);
            }
        }catch(Exception $e){
            return response()->json(['status'=> Config::get('app.status_codes.NP_INVALID_REQUEST'), 'message'=>'User update Error', 'data'=>json_decode("{}")]);
        }
    }
  ...
  public function clearData(){
    ...
    /**
     * Use for update user device id.
     * @param Request $request (user_id, new_device_id, old_device_id, deviceType => IOS/Android, )
     * @return type
     */
    public function deviceTokenUpdate(Request $request) {
        ...
        try {
			...
			if($request->new_device_id != '' && $request->deviceType != '') {
                
				if($request->user_id > 0 ) {
                    
                    if($request->notificationSetting == 1) {
                        $count_device_token = DB::table('users')->where('device_id', $request->new_device_id)->count();

                        if(!$count_device_token) {
                            //update in user table
                            User::where('id', $request->user_id)->update(['device_id' => $request->new_device_id, 'deviceType' => $request->deviceType, 'notificationSetting' => 1]);
                            DB::commit();

                            //remove from general_device_tokens table
                            GeneralDeviceToken::where("device_id", $request->old_device_id)->delete();
                        }
                        return response()->json(["status"=>Config::get('app.status_codes.NP_STATUS_SUCCESS'), "message"=> 'Request processed successfully.', "data"=> json_decode("{}")]);
                        
                    } else {
                        //for notification setting off
                        //update in user table
                        User::where('id', $request->user_id)->update(['device_id' => '', 'deviceType' => '', 'notificationSetting' => 0]);
                        DB::commit();
                    ...
                    //add code for check and insert record in general_device_tokens table
                    $device_token_count = GeneralDeviceToken::where('device_id', $request->new_device_id)->count();
                    
                    if($request->notificationSetting == 1) {
                        if(!$device_token_count) {
                            $token = new GeneralDeviceToken;

                            $token->device_id = $request->new_device_id ;
                        ...
                        //remove from general_device_tokens table
                        if($device_token_count) {
                            GeneralDeviceToken::where("device_id", $request->new_device_id)->delete();
                            DB::commit();
                        }
                    }
                    return response()->json(["status"=>Config::get('app.status_codes.NP_STATUS_SUCCESS'), "message"=> 'Request processed successfully.', "data"=> json_decode("{}")]);
				...
				return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=> "Device token OR device type missing.", "data"=> json_decode("{}")]);
			}
            
        } catch(Exception $e) {
            return response()->json(['status'=>Config::get('app.status_codes.NP_INVALID_REQUEST'), 'message'=>'Something went wrong.','data'=>json_decode("{}")]);
        }
    }
