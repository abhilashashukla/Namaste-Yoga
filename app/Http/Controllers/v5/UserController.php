...
class UserController extends Controller
{
    ...
    public function authenticate(Request $request)
    {
        ...
        //$validator->errors()
        if($validator->fails()){
          return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=>"invalid input details","data"=>json_decode("{}")]);
        }
        //echo $pwd = Hash::make($request->password).' ------ '.$request->email; die;
        $validationChk = User::where('email',$request->email)->get();
        
        //echo $request->password . '===' . $validationChk[0]->password;exit;


        if($validationChk->count()==0){
          return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=>"Invalid credentials","data"=>json_decode("{}")]);
        }else if($validationChk[0]->status != '1'){
          return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=>"User not verified","data"=>json_decode("{}")]);
            ...
            return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'), "message"=>"YCB not approved","data"=>json_decode("{}")]);
        }else if($validationChk[0]->suspended=='1'){
            return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_ACCOUNT_SUSPENDED'), "message"=>"Account suspended","data"=>json_decode("{}")]);
        }
        ...
        //save login user token into 'users' table (mainly use for single user login).
        User::where('email', $request->email)->update(['current_login_token' => $token ]);
        //DB::commit();
        ...
        return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), 'message'=>$message, 'data'=>$user]);
    }
    ...
    public function suspendAccount(Request $request){
      try{
        ...
        if($user->save()){
          JWTAuth::invalidate(JWTAuth::parseToken());
          return response()->json(['status'=> Config::get('app.status_codes.NP_STATUS_SUCCESS'), 'message'=>'','data'=>json_decode("{}")]);
        }else{
          return response()->json(['status'=> Config::get('app.status_codes.NP_DB_ERROR'), 'message'=>'Unable to suspend','data'=>json_decode("{}")]);
        }
        //JWTAuth::setToken($token)->invalidate();
      }catch(Exception $e){
    ...
    public function ResumeOtpAccount(Request $request){
      try{
          ...
          if(isset($user->id)){
             ...
             if($user->suspended=='0'){
              return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'),"message"=>"Account Already Activated","data"=>json_decode("{}")]);
             }
             ...
             return response()->json(["status"=> Config::get('app.status_codes.NP_DB_ERROR'),"message"=>"Error","data"=>json_decode("{}")]);

          }else{
           return response()->json(["status"=> Config::get('app.status_codes.NP_STATUS_FAIL'),"message"=>"invalid Email details","data"=>json_decode("{}")]);
          }
        //JWTAuth::setToken($token)->invalidate();
      }catch(Exception $e){
        return response()->json(['status'=> Config::get('app.status_codes.NP_INVALID_REQUEST'), 'message'=>'Not able to logout','data'=>json_decode("{}")]);
