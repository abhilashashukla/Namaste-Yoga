...
class Aasana extends Controller
{
   ...
   /**
    ...
    public function getSubCategory(Request $request)
    {
        ...
        $data['subcategory'] = AasanaSubCategory::where("aasana_categories_id",$request->aasana_categories_id)->where('status','1')
    ...
    public function SaveAasana(Request $request)
    {
        $user  = Auth::user()->id;
           $validator = Validator::make($request->all(),[
                ...
                'aasana_name' => 'required|max:150',
				'aasana_name' => [											
											Rule::unique('aasanas')->where(function ($query) use($request) {
											return $query->where('aasana_categories_id', $request->aasana_categories_id)->where('aasana_sub_categories_id',$request->aasana_sub_categories_id);
											}),
											], 
                'aasana_description' => 'required',
                //'aasana_image' => 'required|image|mimes:jpeg,png,jpg,gif,svg|max:1500',
                'assana_tag'=>'required|max:150', 
                'assana_video_id' =>'required|max:100',
                'assana_video_duration'=>'required|max:15',
                ...
                [
				  ...
				  'aasana_sub_categories_id.required'=>'The aasana sub category name field is required',
            ...
            else
            {
                ...
                'aasana_name'=>ucwords(trim($request->aasana_name)),
                'aasana_description'=>ucfirst(trim($request->aasana_description)),
                //'aasana_image'=>trim($aasana_image),
                'assana_tag'=>ucwords(trim($request->assana_tag)),
                'assana_video_id'=>trim($request->assana_video_id),
                'assana_video_duration'=>trim($request->assana_video_duration),
                'assana_benifits'=>ucfirst(trim($request->assana_benifits)),
                'assana_instruction'=>ucfirst(trim($request->assana_instruction)),
                'status'=>'1',
                ...
                if($result)
                {
					return redirect()->action('Aasana@ListAasana')->with('success', 'Aasana Added successfully');
                    //return back()->with('success', 'Aasana Added successfully');
                }
                else
                {
                    return back()->withErrors('errors', 'Failed to add Aasana');
                }
    ...
    public function ChangeAasanaStatus(Request $request){
      ...
      public function ViewAasana(Request $request)
      {
			...
			{
				return redirect()->action('Aasana@ListAasana')->with('flash_message_for_aasana_view', 'Aasana not available')->with('flash_type', 'alert-danger'); 
			}
      ...
      public function EditAasana(Request $request)
      {
        ...
        //die;
        $categorylist = AasanaCategoryModel::where('status','1')->get(); 
        $subcategorylist = AasanaSubCategory::where('status','1')->get(); 
        // print_r($category);
        // die;
		if($editaasana)
		{
		...
		{
				return redirect()->action('Aasana@ListAasana')->with('flash_message_for_aasana_edit', 'Aasana not available')->with('flash_type', 'alert-danger'); 
		}
     ...
     public function getEditSubCategory(Request $request)
     {
        // print_r($request->aasana_categories_id);
         $data['subcategory'] = AasanaSubCategory::where("aasana_categories_id",$request->aasana_categories_id)->
                    where('status','1')->get(["subcategory_name","id"])->toArray();
        //  echo "<pre>";
        //  print_r($data['subcategory']);
        //  die;
         return response()->json($data);
