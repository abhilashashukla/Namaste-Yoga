...
<div class="right_col" role="main">
    ...
    <script>
                ...
                },*/
                {
                    ...
                    'render': function(data,type,row) {
                        ...
                        @if(Auth::user()->role_id==4) 
                            if(row.status == '0') {
                                html = '<input type="radio" value="0" onclick="changeStatus('+row.id+', 0)" checked> Pending | ';
                                html += '<input type="radio" name="approve" value="1" onclick="changeStatus('+row.id+', 1)"> Approve | ';
                                html += '<input type="radio" name="reject" value="2" onclick="changeStatus('+row.id+', 2)"> Reject  ';
                            }   
                        @else
                            html = (row.status=='1') ? 'Approved' : 'Not-Approved';
                        @endIf              
                        return html;
                    }              
                }
        ...
        $(document).on('click','#continue1',function(){
            ...
            var hidden_userid = $('#hidden_id').val();
            var hidden_status = $('#hidden_status').val();
            
            jQuery.ajax({
                url: "{{ url('users/changesCenterStatus') }}",
                type: "POST",
                headers: {
                    'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content'),
                },
                data: {
                    "status": hidden_status, "userid": hidden_userid
                },
