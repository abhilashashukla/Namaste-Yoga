...
var table = '';
            ...
            'columns': [
			  ...
			  }, 
              {
                  ...
                  'render': function(data,type,row){
                    ...
                    return '<a class="popoverData" data-content="'+row.quiz_name+'" rel="popover" data-placement="bottom" data-original-title="Name" data-trigger="hover">'+quiz_name+'</a>';
                  }
              },
              {
                    ...
                    @if(Auth::user()->role_id==4)
                      if(row.status==1){
                        html = '<i class="fa fa-toggle-on" onclick="changeStatus('+row.id+',0,'+row.is_editable+')" style="color:green;font-size:20px;" title="De-active"></i>';
						  ...
						  var OCEvent ='';
						  var title = 'title="Quiz Expired"';
						  if(row.is_publishable==1){
							   OCEvent = 'onclick="changeStatus('+row.id+',1,'+row.is_editable+')"';
							   title = 'title="Active"';
						  }
                        html = '<i class="fa fa-toggle-off" '+OCEvent+' style="color:red;font-size:20px;" '+title+'></i>';
                      }  
                    @else
              ...
              },			  
               {
                 ...
                 'render': function(data, type, row) {
				   ...
				   buttonHtml += '<a href="/quiz/edit/' + row.id + '" class="btn btn-success" title="Edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a> <button type="button" id="' + row.id + '" class="btn btn-danger deleteQuiz" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></button>';
					 ...
					 if(row.total_no_of_submission>0){
        ...
        function changeStatus(quiz_id,val,is_editable){ 
			if(val==1 && is_editable==1){
				var msg = '<p>After activating this quiz, You can not make changes to this quiz again, do you want to continue?</p>';
			}else if(val==0){
				var msg = '<p>Are you sure, you want to de-activate this quiz?</p>';
			}else{
				var msg = '<p>Are you sure, you want to activate this quiz?</p>';
			}
			$('#confirm').modal({
				  backdrop: 'static',
				  keyboard: false
				}).find('.modal-body').html(msg+'<input type="hidden" id="hidden_val" value="'+val+'"><input type="hidden" id="hidden_quiz_id" value="'+quiz_id+'">');
			
        }
	 $(document).on('click','#continue1',function(){
		 var val = $('#hidden_val').val();
		 var quiz_id = $('#hidden_quiz_id').val();
		jQuery.ajax({
			  ...
			  success: function(response) {
				...
				if(response.status==1){
					...
					setTimeout(function(){
						$('#msgsection').html('<p id="msgsection" class="alert '+response.flash_type+'">'+response.message+'</p>');
					},1000);
